## Introduction
- This folder contains the Dockerized version of BAP ONIX installation for ION networks. This can be used to install it on staging servers. There is an alternative version which uses local tunnel in bapONIXlt folder which can be used to install on local machines which cannot natively host a server. 
...
5. Run docker compose to bring up the adpater and support services. (Use docker-compose-BAPAdapter-LT.yml below if using local tunnel setup)
```
